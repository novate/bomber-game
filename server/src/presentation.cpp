...
bool PresentationLayer::check_passwordFormat(unsigned char *password)
{
    ...
    if (!strcmp((char *)password, "123456"))
        return true;
    int count = 0;
    while ((*ptr) != '\0')
    {
...
StatusCode PresentationLayer::unpack_DataPacket(Client *client)
{
    ...
    while (client->recv_buffer.has_complete_packet())
    {
        ...
        //string unpacking
        if ((packet.type == PacketType::Info) || (packet.type == PacketType::Password) || (packet.type == PacketType::SendInvit))
        {
...
//function:
//   this will return a 5x5 plane in upward
