...
StatusCode TransferLayer::try_recv(Client &client)
{
    ...
    if (client.recv_buffer.size() < kHeaderSize)
    { // doesn't have header currently
...
int TransferLayer::accept_new_client(int listener)
{
    ...
    else
    {
        ...
        std::cout << "New connection from " << inet_ntop(remoteaddr.ss_family, get_in_addr((struct sockaddr *)&remoteaddr), remoteIP, INET6_ADDRSTRLEN)
